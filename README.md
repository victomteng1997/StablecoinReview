...
```bash
npm install
npm run dev       # http://localhost:4321/StablecoinReview/
...
This repo is prepared for SEO coverage around stablecoins. Starter articles live under `src/content/reviews/` and use review/comparison schema, RSS, JSON feed, sitemap, and base-path-safe internal links.
...
Push this project to a GitHub repository named `StablecoinReview` and enable GitHub Pages from Actions. The workflow at `.github/workflows/deploy.yml` builds and publishes `dist/` automatically.
